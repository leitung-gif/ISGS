...
gl_FragColor = vec4(clamp(col,0.0,1.0), 1.0); }`;
  ...
  /* ─── CTA SECTION — Red/Gold WebGL Gradient (ALL pages) ─── */
  document.querySelectorAll('.cta-section').forEach(section => {
    ...
    let W, H, isVis = false;
    function resizeCTA() {
      W = cvs.width = cvs.clientWidth * Math.min(devicePixelRatio, 1.5);
      H = cvs.height = cvs.clientHeight * Math.min(devicePixelRatio, 1.5);
      gl.viewport(0, 0, W, H);
    }
...
float noise(vec2 p) { vec2 i = floor(p); vec2 f = fract(p); vec2 u = f*f*f*(f*(f*6.0-15.0)+10.0); return mix(mix(hash(i),hash(i+vec2(1,0)),u.x),mix(hash(i+vec2(0,1)),hash(i+vec2(1,1)),u.x),u.y); }
float fbm(vec2 p, int oct) { float v = 0.0, a = 0.55; for(int i = 0; i < 8; i++) { if(i >= oct) break; v += a * noise(p); p = p * 1.95 + vec2(0.9, 1.4); a *= 0.48; } return v; }
vec3 swissGlow(float t) {
  vec3 deepDark = vec3(0.06, 0.03, 0.04);
  vec3 maroon   = vec3(0.18, 0.06, 0.08);
  vec3 swissRed = vec3(0.42, 0.09, 0.10);
  vec3 ember    = vec3(0.52, 0.18, 0.12);
  vec3 gold     = vec3(0.55, 0.42, 0.18);
  vec3 cream    = vec3(0.65, 0.55, 0.35);
  if(t<0.2) return mix(deepDark,maroon,t*5.0);
  else if(t<0.4) return mix(maroon,swissRed,(t-0.2)*5.0);
  else if(t<0.6) return mix(swissRed,ember,(t-0.4)*5.0);
  else if(t<0.8) return mix(ember,gold,(t-0.6)*5.0);
  else return mix(gold,cream,(t-0.8)*5.0);
}
void main() {
  vec2 uv = gl_FragCoord.xy / u_res; uv.y = 1.0 - uv.y;
  vec2 q = vec2(fbm(uv*2.2+vec2(u_time*0.03,u_time*0.04),5),fbm(uv*2.2+vec2(2.3,7.1)+vec2(u_time*0.035,-u_time*0.025),5));
  vec2 r = vec2(fbm(uv*1.8+3.0*q+vec2(2.1,8.7)+u_time*0.02,5),fbm(uv*1.8+3.0*q+vec2(7.4,3.1)+u_time*0.025,5));
  float f = fbm(uv*1.8+3.0*r,5); f = smoothstep(0.05,0.95,f);
  vec3 col = swissGlow(clamp(f,0.0,1.0));
  vec2 vp = uv - 0.5; float vign = 1.0 - dot(vp,vp)*1.4; col *= clamp(vign,0.0,1.0);
  float grain = hash(uv+vec2(u_time*0.013))*0.015-0.0075; col += grain;
  gl_FragColor = vec4(clamp(col,0.0,1.0), 1.0);
}`;
